...
async function testRangePartitioning() {
  ...
  try {
    ...
    console.log(`Created ${partitions.length} partitions:`, partitions.map(p => p.name));
    ...
    for (const criteria of testQueries) {
      ...
      // Execute the query
      const result = await service.queryPartitioned(
        rangeConfig.name,
        criteria,
        queryPartition
      );
      
      console.log(`Query returned ${result.results.length} results from ${result.partitionsQueried.length} partitions`);
...
async function testTemporalPartitioning() {
  ...
  try {
    ...
    console.log('Active partitions after retention policy:',
      updatedMetadata.filter(p => p.active).map(p => p.name)
    );
    console.log('Inactive partitions after retention policy:',
      updatedMetadata.filter(p => !p.active).map(p => p.name)
    );
  } catch (error) {
