...
const mockMCPService = {
  ...
  executeTool: async (toolName, parameters) => {
    ...
    console.log(`MOCK MCP: Using tool name '${actualToolName}', parameters:`, JSON.stringify(parameters, null, 2));
    ...
    if (actualToolName === 'market.getLandUseImpact') {
      return {
        ...
        result: {
          ...
          nearbyDevelopments: [
            { type: 'Commercial', distance: 0.5, impact: 'neutral' }
          ],
  ...
  executeRequest: async (request) => {
    if (request && request.tool) {
      console.log(`MOCK MCP: Execute request for tool '${request.tool}', parameters:`, JSON.stringify(request.parameters || {}, null, 2));
      ...
      console.log(`MOCK MCP: Execute request for toolName '${request.toolName}', parameters:`, JSON.stringify(request.parameters || {}, null, 2));
  ...
  try {
    ...
    try {
      ...
      if (resultData.confidenceIntervals && resultData.confidenceIntervals.length > 0) {
        const lastInterval = resultData.confidenceIntervals[resultData.confidenceIntervals.length - 1];
