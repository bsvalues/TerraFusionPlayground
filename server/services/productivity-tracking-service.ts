...
export class ProductivityTrackingService {
   ...
   */
  async getProductivityMetrics(userId: number, startDate?: Date, endDate?: Date): Promise<DeveloperProductivityMetric[]> {
    try {
      let query = db.select().from(developerProductivityMetrics).where(eq(developerProductivityMetrics.userId, userId));
  ...
  async getTodayProductivityMetric(userId: number): Promise<DeveloperProductivityMetric | null> {
    try {
      ...
      const result = await db.select().from(developerProductivityMetrics)
        .where(and(
      ...
      const newMetric = await db.insert(developerProductivityMetrics)
   ...
   */
  async updateProductivityMetric(id: number, data: Partial<InsertDeveloperProductivityMetric>): Promise<DeveloperProductivityMetric | null> {
    try {
      const updated = await db.update(developerProductivityMetrics)
        .set({ 
          ...data, 
          updatedAt: new Date() 
        })
   ...
   */
  async startActivitySession(data: InsertDeveloperActivitySession): Promise<DeveloperActivitySession | null> {
    try {
      const session = await db.insert(developerActivitySessions)
        .values(data)
        .returning();
  ...
  async endActivitySession(id: number): Promise<DeveloperActivitySession | null> {
    try {
      const now = new Date();
      const sessions = await db.select().from(developerActivitySessions)
      ...
      const updated = await db.update(developerActivitySessions)
        .set({ 
          endTime: now,
      ...
      if (session.metricId) {
        ...
        await this.updateMetricsBasedOnActivity(session.metricId, session.activityType, durationMinutes, session.codeLines || 0);
      }
   ...
   */
  async getRecentActivitySessions(userId: number, limit: number = 10): Promise<DeveloperActivitySession[]> {
    try {
      return await db.select().from(developerActivitySessions)
   ...
   */
  async getEnergyLevelRecommendations(userId: number, energyLevel: DeveloperEnergyLevel): Promise<EnergyLevelRecommendation | null> {
    try {
      const recommendations = await db.select().from(energyLevelRecommendations)
        .where(and(
  ...
  async getProductivityTrendData(userId: number, days: number = 30): Promise<any> {
    try {
      ...
      const tasksByDay: Array<{date: string, completed: number, inProgress: number, blocked: number}> = [];
   ...
   */
  private async createDefaultRecommendation(userId: number, energyLevel: DeveloperEnergyLevel): Promise<EnergyLevelRecommendation | null> {
    try {
      ...
      switch (energyLevel) {
        case DeveloperEnergyLevel.HIGH:
          ...
          avoidActivities = [
            ...
            ProductivityMetricType.MEETING
          ];
          strategies = [
      ...
      const created = await db.insert(energyLevelRecommendations)
  ...
  private async updateMetricsBasedOnActivity(
    ...
    try {
      const metrics = await db.select().from(developerProductivityMetrics)
      ...
      await db.update(developerProductivityMetrics)
        .set({ 
          productiveHours,
          completedTasks,
