...
export class DataLineageTracker {
  ...
  private shouldTrackField(field: string): boolean {
    ...
    const skipFields = [
      ...
      '_metadata', '_internal', '_version', '_hash'
    ];
  ...
  private areValuesEqual(oldValue: any, newValue: any): boolean {
    ...
    if ((oldValue === null || oldValue === undefined) && (newValue !== null && newValue !== undefined)) return false;
    ...
    try {
      ...
      const mockHistory: DataLineageRecord[] = [
        ...
        },
        {
          ...
          fieldChanges: [
            { field: 'value', oldValue: '250000', newValue: '275000' }
          ],
      ...
      if (options.sourceTypes && options.sourceTypes.length > 0) {
        filteredHistory = filteredHistory.filter(
          record => options.sourceTypes!.includes(record.sourceType)
        );
      }
      
      if (options.operationTypes && options.operationTypes.length > 0) {
        filteredHistory = filteredHistory.filter(
          record => options.operationTypes!.includes(record.operationType)
        );
      }
      
      if (options.fields && options.fields.length > 0) {
        filteredHistory = filteredHistory.filter(record => 
          record.fieldChanges.some(change => 
            options.fields!.includes(change.field)
          )
        );
      }
    ...
    try {
      ...
      const mockChanges: DataLineageRecord[] = [
        ...
        },
        {
          ...
          operationType: ChangeOperationType.UPDATE,
          fieldChanges: [
            { field: 'propertyType', oldValue: 'RES', newValue: 'COM' }
          ]
        }
      ];
  ...
  async generateLineageReport(entityType: string, entityId: string): Promise<any> {
    try {
      ...
      return {
        ...
        firstChangeTimestamp: history.length > 0 ? history[history.length - 1].changeTimestamp : null,
