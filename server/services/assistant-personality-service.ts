...
class AssistantPersonalityService {
   ...
   */
  async createPersonality(personalityData: InsertAssistantPersonality): Promise<AssistantPersonality> {
    try {
      logger.info(`AssistantPersonalityService: Creating new assistant personality: ${personalityData.name}`);
  ...
  async updatePersonality(
    ...
    try {
      ...
      const [updatedPersonality] = await db
        ...
        .where(and(
          eq(assistantPersonalities.id, id),
          eq(assistantPersonalities.userId, userId)
        ))
        .returning();
  ...
  generateSystemPrompt(personality: AssistantPersonality): string {
    ...
    if (!prompt || prompt.trim() === '') {
      ...
      if (traits.formality > 7) {
        prompt += 'Communicate in a formal, professional manner. Use proper terminology and maintain a respectful tone. ';
      } else if (traits.formality < 4) {
        prompt += 'Communicate in a casual, conversational manner. Use simple language and a friendly tone. ';
      }
      ...
      if (traits.creativity > 7) {
        prompt += 'Feel free to suggest innovative approaches and think outside the box when appropriate. ';
      ...
      const defaultTemplates: InsertPersonalityTemplate[] = [
        {
          ...
          description: 'A formal, precise assistant focused on technical correctness and professional communication',
          ...
          systemPrompt: 'You are a professional property assessment assistant with expertise in valuation methodologies and property tax regulations. Communicate with precision and provide technically accurate information. Maintain a formal, professional tone at all times.',
          exampleMessages: [
        ...
        },
        {
          ...
          systemPrompt: 'You are a friendly guide to the property assessment system. Your goal is to make complex concepts easy to understand for everyone. Use conversational language, helpful examples, and a warm, supportive tone. Avoid jargon when possible and explain technical terms when you need to use them.',
          exampleMessages: [
        ...
        },
        {
          ...
          description: 'A detailed, technically-focused assistant for in-depth analysis and explanations',
        ...
        },
        {
          ...
          description: 'An innovative assistant that provides alternative perspectives and creative solutions',
        ...
        },
        {
          ...
          systemPrompt: 'You are a concise assistant for property assessment professionals. Provide brief, efficient responses that deliver maximum value with minimum text. Focus on the most important points, eliminate unnecessary details, and get straight to the point. Use bullet points, short sentences, and clear language.',
          exampleMessages: [
