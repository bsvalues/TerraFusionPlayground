  ...
  async generateEqualizationReport(
    ...
    try {
      ...
      for (const [propertyType, stats] of Object.entries(propertyTypeStats)) {
        ...
        const meanRatio = validSamples.length > 0 
        ...
        if (validSamples.length > 1) {
          const variance = validSamples.reduce(
            (sum, s) => sum + Math.pow(s.ratio - meanRatio, 2), 
            0
          ) / validSamples.length;
        ...
        const isRatioCompliant = meanRatio >= 0.90 && meanRatio <= 1.10;
      ...
      const isCountyCompliant = countyRatio >= 0.90 && countyRatio <= 1.10;
      ...
      logger.error(`Error generating equalization report: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async generateRevaluationCycleReport(
    ...
    try {
      ...
      for (const property of properties) {
        ...
        if (!areaStats[areaId].lastRevaluationDate || 
      ...
      logger.error(`Error generating revaluation cycle report: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async generateExemptionVerificationReport(
    ...
    try {
      ...
      for (const [exemptionType, stats] of Object.entries(exemptionTypeStats)) {
        ...
        const verificationPercentage = stats.count > 0 
          ? (stats.verifiedCount / stats.count) * 100 
          : 100;
        
        // Determine required verification percentage
        const requiredPercentage = requiredVerificationPercentages[exemptionType] || 
          requiredVerificationPercentages.DEFAULT;
      ...
      const totalVerificationPercentage = totalCount > 0 
        ? (totalVerifiedCount / totalCount) * 100 
        : 100;
      
      // Check overall compliance (all types compliant and overall at least 95%)
      const allTypesCompliant = Object.values(exemptionTypeResults)
        .every(result => result.isCompliant);
      ...
      // Log and return
      logger.info(`Exemption verification report generated for tax year ${taxYear}: ID ${report.id}`);
      ...
      logger.error(`Error generating exemption verification report: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async generateAppealComplianceReport(
    ...
    try {
      ...
      for (const appeal of appeals) {
        ...
        if (appeal.status === "scheduled" || appeal.status === "heard" || appeal.status === "decided") {
          appealsWithHearings++;
      ...
      const averageResolutionDays = resolvedAppeals > 0 
        ? totalResolutionDays / resolvedAppeals 
        : 0;
      const percentResolvedWithin45Days = resolvedAppeals > 0 
        ? (resolvedWithin45Days / resolvedAppeals) * 100 
        : 100;
      const percentWithHearingScheduledOnTime = appealsWithHearings > 0 
        ? (hearingsScheduledOnTime / appealsWithHearings) * 100 
        : 100;
        
      // Check timeline compliance (90% within requirements)
      const isTimelineCompliant = percentResolvedWithin45Days >= 90 && 
        percentWithHearingScheduledOnTime >= 90;
      
      // Calculate hearing compliance metrics
      const percentOnTimeNotices = totalHearings > 0 
        ? (onTimeNotices / totalHearings) * 100 
        : 100;
      const percentProperEvidence = totalHearings > 0 
        ? (properEvidenceExchange / totalHearings) * 100 
        : 100;
      const percentProperDocumentation = totalHearings > 0 
        ? (properlyDocumented / totalHearings) * 100 
        : 100;
        
      // Check hearing compliance (all metrics above 95%)
      const isHearingCompliant = percentOnTimeNotices >= 95 && 
      ...
      const report: AppealComplianceReport = {
        ...
        timelinessMetrics: {
          ...
          percentWithHearingScheduledOnTime: parseFloat(percentWithHearingScheduledOnTime.toFixed(1)),
      ...
      if (!isTimelineCompliant || !isHearingCompliant) {
        ...
        if (!isTimelineCompliant) {
          issues.push(`Appeal resolution timelines (${percentResolvedWithin45Days.toFixed(1)}% within 45 days, required: 90%)`);
        }
        
        if (!isHearingCompliant) {
          if (percentOnTimeNotices < 95) {
            issues.push(`Hearing notices (${percentOnTimeNotices.toFixed(1)}% on time, required: 95%)`);
          }
          if (percentProperEvidence < 95) {
            issues.push(`Evidence exchange (${percentProperEvidence.toFixed(1)}% proper, required: 95%)`);
          }
          if (percentProperDocumentation < 95) {
            issues.push(`Hearing documentation (${percentProperDocumentation.toFixed(1)}% proper, required: 95%)`);
          }
        }
      ...
      logger.error(`Error generating appeal compliance report: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async generateAnnualCompliancePackage(
    ...
    try {
      ...
      if (!equalizationReport.countyTotal.isCompliant) {
        complianceIssues.push(`Equalization ratio (${equalizationReport.countyTotal.ratio}) outside required range (0.90-1.10)`);
      }
      ...
      if (!revaluationReport.isCompliant) {
        ...
        complianceIssues.push(`Revaluation cycle: areas outside ${revaluationReport.cycleYears}-year cycle (${nonCompliantAreas})`);
      }
      ...
      if (!appealReport.timelinessMetrics.isCompliant) {
        complianceIssues.push(`Appeal timeline issues: ${appealReport.timelinessMetrics.percentResolvedWithin45Days}% resolved within 45 days (required: 90%)`);
      }
      
      if (!appealReport.hearingComplianceMetrics.isCompliant) {
        const hearingIssues = [];
        const hearingMetrics = appealReport.hearingComplianceMetrics;
        const percentOnTimeNotices = hearingMetrics.totalHearings > 0 
          ? (hearingMetrics.onTimeNotices / hearingMetrics.totalHearings) * 100 
          : 100;
        const percentProperEvidence = hearingMetrics.totalHearings > 0 
          ? (hearingMetrics.properEvidenceExchange / hearingMetrics.totalHearings) * 100 
          : 100;
        const percentProperDocumentation = hearingMetrics.totalHearings > 0 
      ...
      logger.error(`Error generating annual compliance package: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  private async getPropertySampleForRatioStudy(
    ...
    try {
      ...
      for (const propertyType of propertyTypes) {
        const propertiesOfType = eligibleProperties.filter(
      ...
      logger.error(`Error getting property sample: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
