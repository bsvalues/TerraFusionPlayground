...
export class IngestionAgent extends BaseAgent {
      ...
      capabilities: [
        // Define core capabilities
        {
          ...
          handler: async (parameters, agent) =>
            await this.importFromFTP(parameters.remotePath, parameters.dataType),
        },
        {
        ...
        },
        {
          ...
          handler: async (parameters, agent) =>
            await this.loadValidatedData(parameters.importId, parameters.options),
        },
        {
          name: 'exportToFTP',
          ...
          handler: async (parameters, agent) =>
            await this.exportToFTP(parameters.dataType, parameters.filter, parameters.remotePath),
        },
        {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      await this.logActivity(
        ...
        `FTP connection test failed: ${error.message}`
      );
  ...
  private async importFromFTP(remotePath: string, dataType: string): Promise<any> {
    try {
      ...
      await this.logActivity(
        'ftp_import',
        `Starting import of ${dataType} from FTP: ${remotePath}`
      );
      ...
      await this.logActivity(
        'ftp_import_error',
        `Error importing ${dataType} from FTP: ${error.message}`
      );
      ...
      try {
        ...
        await this.logActivity(
          'import_update_error',
          `Failed to update import record with error: ${updateError.message}`
        );
      }
  ...
  private async validateImportData(importId: string): Promise<any> {
    try {
      ...
      if (importRecord.status !== 'parsed') {
        throw new Error(
          `Import ${importId} is not in 'parsed' state (current: ${importRecord.status})`
        );
      }
      ...
      await this.logActivity(
        ...
        `Error validating import ${importId}: ${error.message}`
      );
  ...
  private async loadValidatedData(importId: string, options: any = {}): Promise<any> {
    try {
      ...
      if (importRecord.status !== 'validated') {
        throw new Error(
          `Import ${importId} is not in 'validated' state (current: ${importRecord.status})`
        );
      }
      ...
      await this.logActivity(
        'import_load_error',
        `Error loading data for import ${importId}: ${error.message}`
      );
  ...
  private async exportToFTP(dataType: string, filter: any = {}, remotePath?: string): Promise<any> {
    try {
      ...
      await this.logActivity(
        'ftp_export',
        `Successfully exported ${dataType} to FTP: ${remotePath}`,
        {
          recordCount: data.length,
          fileSize: csvContent.length,
        }
      ...
      await this.logActivity(
        'ftp_export_error',
        `Error exporting ${dataType} to FTP: ${error.message}`
      );
      ...
      try {
        ...
        await this.logActivity(
          'export_update_error',
          `Failed to update export record with error: ${updateError.message}`
        );
      }
  ...
  private async checkImportStatus(importId: string): Promise<any> {
    try {
      ...
      await this.logActivity(
        'import_status_check',
        `Checked status of import ${importId}: ${importRecord.status}`
      );
      ...
      await this.logActivity(
        'import_status_error',
        `Error checking import status for ${importId}: ${error.message}`
      );
      throw error;
    }
  }
    ...
    try {
      ...
      if (csvPath) {
        ...
        try {
          ...
          await this.logActivity(
            'pacs_modules_ftp_error',
            `Error downloading PACS modules from FTP: ${ftpError.message}`
          );
          throw ftpError;
        }
      } else {
        ...
        try {
          ...
          await this.logActivity(
            'pacs_modules_file_error',
            `Error reading default PACS modules file: ${fsError.message}`
          );
      ...
      await this.logActivity(
        'pacs_modules_import_error',
        `Error importing PACS modules: ${error.message}`
      );
      throw error;
    }
  }
