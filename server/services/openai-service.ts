...
export interface IOpenAIService {
  ...
  generateImage(prompt: string, responseFormat?: 'url' | 'b64_json', size?: string): Promise<GeneratedImageResult>;
...
export class OpenAIService implements IOpenAIService {
  ...
  async generateText(
    ...
    try {
      ...
      console.error(`Error in OpenAI text generation: ${errorDetails.type} - ${errorDetails.message}`, error);
  ...
  async generateCompletion(
    ...
    try {
      ...
      // Get detailed error information
      const errorDetails = this.getErrorDetails(error);
      console.error(`Error in OpenAI completion generation: ${errorDetails.type} - ${errorDetails.message}`, error);
  ...
  async generateJson<T>(
    ...
    try {
      ...
      // Get detailed error information
      const errorDetails = this.getErrorDetails(error);
      console.error(`Error in OpenAI JSON generation: ${errorDetails.type} - ${errorDetails.message}`, error);
  ...
  async analyzeImage(
    ...
    try {
      ...
      // Get detailed error information
      const errorDetails = this.getErrorDetails(error);
      console.error(`Error in OpenAI image analysis: ${errorDetails.type} - ${errorDetails.message}`, error);
  ...
  async generateImage(
    ...
    try {
      ...
      // Get detailed error information
      const errorDetails = this.getErrorDetails(error);
      console.error(`Error in OpenAI image generation: ${errorDetails.type} - ${errorDetails.message}`, error);
