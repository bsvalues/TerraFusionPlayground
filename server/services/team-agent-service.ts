...
export class TeamAgentService {
      ...
      },
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'array',
            description:
              'Areas to focus on during review (e.g., accessibility, visual hierarchy, etc.)',
            required: false,
  ...
  public async executeCapability(
    ...
    try {
      ...
      if (teamMember.status !== TeamMemberStatus.AVAILABLE) {
        throw new Error(
          `Team member ${teamMember.name} is currently ${teamMember.status} and cannot execute capabilities`
        );
      }
  ...
  private extractFiles(content: string): { name: string; content: string }[] {
    const filePattern =
      /File:\s*`([^`]+)`\s*```(?:tsx?|jsx?|javascript|typescript)?\n([\s\S]*?)```/g;
    const files = [];
    let match;
   ...
   */
  private extractComponentVariants(
    content: string,
   ...
   */
  private extractAssessmentIssues(
    content: string
  ...
  private extractComplianceStatus(content: string): { compliant: boolean; details: string } {
    ...
    if (match && match[1]) {
      ...
      const compliant =
        ...
        !status.includes('non-compliant') &&
        !status.includes('not compliant');
