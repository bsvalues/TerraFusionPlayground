...
export class VoiceCommandErrorHandler {
  ...
  private commandPatterns: Record<
    string,
    { pattern: RegExp; replacement: string; type: VoiceCommandType }[]
  > = {
    // Navigation patterns
    navigation: [
      {
    ...
    system: [
      {
        pattern:
          /(?:show|display|tell me|what is)\s+(?:the)?\s*(?:help|commands|available commands|what can i say)/i,
  ...
  async handleCommandNotFound(rawCommand: string, contextId?: string): Promise<ErrorResponse> {
    ...
    const examplePhrases = helpContent.flatMap(item => item.examplePhrases || []).slice(0, 5); // Limit to 5 examples

    return {
      status: VoiceCommandStatus.FAILED,
      errorMessage: 'Command not recognized',
      suggestions: ['Try being more specific', 'Use simpler language', 'Speak more clearly'],
      alternativeCommands: correctedCommand
        ? [correctedCommand, ...examplePhrases]
        : examplePhrases,
  ...
  async handleAmbiguousCommand(
    ...
    return {
      ...
      suggestions: ['Try being more specific', 'Include more details in your command'],
