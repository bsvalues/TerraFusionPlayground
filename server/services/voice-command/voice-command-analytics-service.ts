...
export class VoiceCommandAnalyticsService {
  ...
  async logVoiceCommand(logData: InsertVoiceCommandLog): Promise<number> {
    ...
    try {
      ...
      const [result] = await db.insert(voiceCommandLogs)
        .values(logData)
  ...
  private async updateDailyAnalytics(userId: number): Promise<void> {
    ...
    try {
      ...
      const successfulCommands = logs.filter(log => log.status === VoiceCommandStatus.SUCCESS).length;
      const failedCommands = logs.filter(log => log.status === VoiceCommandStatus.FAILED).length;
      const ambiguousCommands = logs.filter(log => log.status === VoiceCommandStatus.AMBIGUOUS).length;
      ...
      if (existingRecord) {
        ...
        await db.update(voiceCommandAnalytics)
          .set(analyticsData)
          .where(eq(voiceCommandAnalytics.id, existingRecord.id));
      } else {
        // Insert new record
        await db.insert(voiceCommandAnalytics)
    ...
    analyticsRecords.forEach(record => {
      ...
      if (record.commandTypeCounts) {
        Object.entries(record.commandTypeCounts).forEach(([type, count]) => {
          aggregated.commandTypeCounts[type] = (aggregated.commandTypeCounts[type] || 0) + (count as number);
        });
      }
  ...
  async generateAnalyticsReport(dateRange: { start: Date, end: Date }): Promise<any> {
    try {
      ...
      const successRate = totalCommands > 0 
        ? Number(((successfulCommands / totalCommands) * 100).toFixed(1)) 
        : 0;
  ...
  private async calculateDailyTrends(dateRange: { start: Date, end: Date }): Promise<any[]> {
    ...
    return dailyData.map(day => ({
      ...
      successRate: day.totalCommands > 0 
