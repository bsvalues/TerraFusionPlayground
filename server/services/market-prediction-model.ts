...
export class MarketPredictionModel {
  ...
  public async predictForProperty(
    ...
    const marketPrediction: MarketPrediction = {
      ...
      annualGrowthRate: this.calculateAnnualGrowthRate(
        currentValue,
  ...
  public async getEconomicForecasts(region: string, years: number = 5): Promise<any> {
    ...
    const forecasts = {
      ...
      homeValueIndex: Array.from({ length: years }, (_, i) => ({
        year: currentYear + i + 1,
        value: this.forecastMetric(
          currentIndicators.historicalData.homeValueIndex[4].value,
          8,
          15,
          i + 1
        ),
  ...
  private async generatePredictions(
    ...
    const medianPriceGrowth =
      marketTrends.find(t => t.metric === 'median_sale_price')?.value || 3.0;
    ...
    if (property.yearBuilt) {
      ...
      propertySpecificGrowth +=
        age < 5 ? 0.5 : age < 10 ? 0.3 : age < 20 ? 0.1 : age < 40 ? -0.1 : -0.3;
    }
