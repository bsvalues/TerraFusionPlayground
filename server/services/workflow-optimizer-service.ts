...
export class WorkflowOptimizerService {
   ...
   */
  async createOptimizationRequest(request: InsertWorkflowOptimizationRequest): Promise<WorkflowOptimizationRequest> {
   ...
   */
  async updateOptimizationRequest(id: number, updates: Partial<InsertWorkflowOptimizationRequest>): Promise<WorkflowOptimizationRequest | undefined> {
  ...
  private async analyzeCodebase(request: WorkflowOptimizationRequest): Promise<{
    ...
    try {
      ...
      const response = await this.openai.chat.completions.create({
        model: OPENAI_MODEL,
        messages: [
          { role: "system", content: "You are an expert software engineer specializing in code analysis and optimization." },
      ...
      const readDir = async (dirPath: string) => {
        ...
        for (const file of files) {
          ...
          if (file.isDirectory()) {
            ...
            if (['.js', '.jsx', '.ts', '.tsx', '.py', '.java', '.go', '.rb', '.php', '.cs', '.c', '.cpp'].includes(ext)) {
              try {
