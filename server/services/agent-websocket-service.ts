...
export class AgentWebSocketService {
  ...
  public initialize(server: Server) {
    try {
      ...
      // Log success
      console.log('[WebSocket Debug] WebSocketServer created successfully for path: /api/agents/ws');
  ...
  private setupAgentProtocolSubscriptions() {
    ...
    const coordinationSub = this.agentProtocol.subscribe('websocket-bridge', 'coordination', (message) => {
    ...
    const capabilitySub = this.agentProtocol.subscribe('websocket-bridge', 'capability', (message) => {
      this.broadcastToType(ClientType.FRONTEND, {
