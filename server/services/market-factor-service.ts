 ...
 */
const MARKET_FACTORS: Record<
  string,
  Omit<MarketFactor, 'currentTrend' | 'historicalData' | 'forecastData'>
> = {
  interest_rates: {
    name: 'Interest Rates',
  ...
  local_economic_growth: {
    ...
    description:
      'Economic growth indicators for the specific region including job growth, wage growth, and business expansion',
    impactWeight: 0.75,
  ...
  infrastructure_development: {
    ...
    description:
      'Planned or ongoing infrastructure projects that may affect property values (roads, public transport, utilities)',
    impactWeight: 0.7,
...
const calculateOverallMarketImpact = (factors: MarketFactor[]): number => {
  ...
  const weightedImpact = factors.reduce((sum, factor) => {
    ...
    const trendValue =
      factor.currentTrend === 'rising' ? 1 : factor.currentTrend === 'stable' ? 0 : -1;
...
export class MarketFactorService {
  ...
  public getMarketFactorReport(zipCode: string, propertyType: string): MarketFactorReport {
    ...
    const marketOutlook: 'positive' | 'negative' | 'neutral' =
      overallImpactScore > 0.2 ? 'positive' : overallImpactScore < -0.2 ? 'negative' : 'neutral';
  ...
  public getTopMarketFactors(
    ...
    return [...report.factors].sort((a, b) => b.impactWeight - a.impactWeight).slice(0, count);
  }
  ...
  public getMarketFactorsForLLM(zipCode: string, propertyType: string): string[] {
    ...
    return topFactors.map(
      factor => `${factor.name} (${factor.currentTrend.toUpperCase()}): ${factor.description}`
    );
  }
  ...
  private generateForecastData(
    factorKey: string,
    currentTrend: 'rising' | 'falling' | 'stable'
  ): { period: string; value: number; confidence: number }[] {
    const result = [];
    const currentYear = new Date().getFullYear();
    ...
    switch (factorKey) {
      ...
      case 'comparable_sales_trends':
        const basePricePerSqFt =
          propertyType === 'Residential' ? 200 + Math.random() * 150 : 300 + Math.random() * 200;
  ...
  private getNationalMarketData(factorKey: string): Record<string, any> {
    ...
    switch (factorKey) {
      ...
      default:
        return {
          nationalTrend:
            Math.random() > 0.3 ? (Math.random() > 0.5 ? 'improving' : 'stable') : 'deteriorating',
  ...
  public getMarketFactorExplanation(zipCode: string, propertyType: string): string {
    ...
    topFactors.forEach(factor => {
      const trendSymbol =
        factor.currentTrend === 'rising' ? '↑' : factor.currentTrend === 'falling' ? '↓' : '→';
