...
export class GISDataService {
  ...
  async getFeatureCollectionsByLayer(layerId: number): Promise<GISFeatureCollection[]> {
    ...
    // Log operation
    if (this.options.logOperations) {
      console.log(`GIS Data Service: Retrieved ${collections.length} feature collections for layer ${layerId}`);
    }
   ...
   */
  async createFeatureCollection(collection: InsertGISFeatureCollection): Promise<GISFeatureCollection> {
    // Validate input if enabled
    if (this.options.validation?.enabled) {
    ...
    // Log operation
    if (this.options.logOperations) {
      console.log(`GIS Data Service: Created feature collection ${createdCollection.id} for layer ${collection.layerId}`);
    }
   ...
   * Sort features
   */
  private sortFeatures(features: any[], orderBy: string, orderDirection: 'asc' | 'desc' = 'asc'): any[] {
   ...
   * Buffer operation
   */
  private bufferOperation(geometries: any[], options: { distance: number }): SpatialOperationResult {
    // In a real implementation, this would use a spatial library
   ...
   * Simplify operation
   */
  private simplifyOperation(geometries: any[], options: { tolerance: number }): SpatialOperationResult {
    // In a real implementation, this would use a spatial library
    // For this example, we just return the input geometries
    return {
      success: true,
      ...
      if (!Array.isArray(collection.data.features)) {
        ...
        collection.data.features.forEach((feature: any, index: number) => {
          if (!feature.type || feature.type !== 'Feature') {
            errors.push(`Feature at index ${index} must have a "type" property with value "Feature"`);
          }
