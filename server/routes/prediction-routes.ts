...
export const createPredictionRoutes = (storage: IStorage) => {
    ...
    try {
      ...
      const predictions = await Promise.all(
        properties.map(async (property) => {
          ...
          const coordinates = typeof property.geometry === 'string' 
      ...
      const validPredictions = predictions.filter(pred => 
        pred.coordinates && pred.coordinates.length > 0
      );
   ...
   */
  async function getPredictedValue(
    propertyId: string, 
    currentValue: number, 
    timeframe: string
  ) {
    ...
    switch (timeframe) {
      case '1year':
        ...
        break;
      case '5year':
        growthMultiplier = 1.30; // 30% growth over 5 years
        break;
      case '10year':
        growthMultiplier = 1.70; // 70% growth over 10 years
        break;
    }
