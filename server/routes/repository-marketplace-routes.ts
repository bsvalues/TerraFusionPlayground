...
const insertRepositorySchema = createInsertSchema(repositories).omit({ id: true, createdAt: true, updatedAt: true, stars: true, forks: true, downloads: true, featured: true });
const insertVersionSchema = createInsertSchema(repositoryVersions).omit({ id: true, createdAt: true, updatedAt: true, downloads: true });
const insertReviewSchema = createInsertSchema(repositoryReviews).omit({ id: true, createdAt: true, updatedAt: true });
const insertDependencySchema = createInsertSchema(repositoryDependencies).omit({ id: true, createdAt: true, updatedAt: true });
...
router.patch('/repositories/:repositoryId/versions/:versionId', async (req, res) => {
  try {
    ...
    const version = await repositoryMarketplaceService.updateRepositoryVersion(versionId, validatedData);
...
router.patch('/repositories/:repositoryId/reviews/:reviewId', async (req, res) => {
  try {
    ...
    const review = await repositoryMarketplaceService.updateRepositoryReview(reviewId, validatedData);
...
router.get('/repositories/:repositoryId/dependencies', async (req, res) => {
  try {
    ...
    const dependencies = await repositoryMarketplaceService.getRepositoryDependencies(repositoryId);
...
router.post('/repositories/:repositoryId/dependencies', async (req, res) => {
  try {
    ...
    const dependencyRepo = await repositoryMarketplaceService.getRepositoryById(validatedData.dependencyRepoId);
    
    if (!dependencyRepo) {
      return res.status(404).json({ error: 'Dependency repository not found' });
    }
    
    const dependency = await repositoryMarketplaceService.createRepositoryDependency(validatedData);
...
router.patch('/repositories/:repositoryId/dependencies/:dependencyId', async (req, res) => {
  try {
    ...
    const dependency = await repositoryMarketplaceService.updateRepositoryDependency(dependencyId, validatedData);
