...
const { describe, test, expect, beforeEach, jest } = await import('@jest/globals');
...
describe('ConnectionManager', () => {
  ...
  test('should implement exponential backoff for reconnection attempts', () => {
    ...
    const errorHandler = wsInstance.addEventListener.mock.calls.find(call => call[0] === 'error')[1];
    ...
    const errorHandler2 = wsInstance2.addEventListener.mock.calls.find(call => call[0] === 'error')[1];
    ...
    const errorHandler3 = wsInstance3.addEventListener.mock.calls.find(call => call[0] === 'error')[1];
    ...
    const errorHandler4 = wsInstance4.addEventListener.mock.calls.find(call => call[0] === 'error')[1];
  ...
  test('should update UI indicators correctly during connection state changes', () => {
    ...
    const closeHandler = wsInstance.addEventListener.mock.calls.find(call => call[0] === 'close')[1];
    closeHandler();
    ...
    const sseErrorHandler = sseInstance.addEventListener.mock.calls.find(call => call[0] === 'error')[1];
