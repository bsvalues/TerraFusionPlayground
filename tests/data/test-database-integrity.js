...
async function checkDataIntegrity() {
  ...
  console.log(
    `API returned ${propertiesResult.data.length} properties and ${agentsResult.data.length} agents`
  );
  ...
  try {
    ...
    try {
      ...
      if (propertiesCount !== propertiesResult.data.length) {
        console.error(
          `Property count mismatch: API has ${propertiesResult.data.length}, DB has ${propertiesCount}`
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (agentsCount !== agentsResult.data.length) {
        console.error(
          `Agent count mismatch: API has ${agentsResult.data.length}, DB has ${agentsCount}`
        );
      }
    } catch (error) {
