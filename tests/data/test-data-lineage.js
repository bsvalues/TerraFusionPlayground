...
async function testDataLineage() {
  ...
  const firstUpdateResult = await makeRequest(
  ...
  const secondUpdateResult = await makeRequest(
    `/properties/${createdProperty.id}`,
    'PATCH',
    secondUpdate
  );
  ...
  const fieldLineage = await makeRequest(
    `/data-lineage/property/${createdProperty.propertyId}/field/value`
  );
