...
export class RobustWebSocketManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.socket && this.socket.readyState === WebSocket.OPEN) {
        try {
          ...
          if (!binary && typeof data !== 'string' && !(data instanceof ArrayBuffer) && !(data instanceof Blob)) {
  ...
  private handleClose(event: CloseEvent, reject: Function): void {
    ...
    this.log(`WebSocket connection closed: Code ${event.code}, Reason: ${event.reason || 'No reason provided'}, Clean: ${event.wasClean}`);
    ...
    reject(new Error(`WebSocket closed: Code ${event.code}, Reason: ${event.reason || 'No reason provided'}`));
  }
  ...
  private attemptReconnect(): void {
    ...
    const delay = Math.min(
      this.options.reconnectInterval! * Math.pow(this.options.reconnectMultiplier!, this.reconnectAttempt - 1),
      this.options.maxReconnectInterval!
    );
    
    this.log(`Attempting to reconnect in ${delay}ms (attempt ${this.reconnectAttempt}/${this.options.maxReconnectAttempts})`);
    ...
    this.reconnectTimer = setTimeout(() => {
      ...
      this.connect().then(() => {
        ...
        if (this.reconnectAttempt >= this.options.maxReconnectAttempts! && this.options.fallbackPolling) {
          this.log('Max reconnect attempts reached, falling back to polling');
          this.initPolling();
        }
      });
   ...
   */
  private bufferMessage(data: any, priority: MessagePriority, maxRetries: number, binary: boolean): void {
    // Add to buffer
