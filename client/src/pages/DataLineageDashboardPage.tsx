...
export function DataLineageDashboardPage() {
  ...
  const filteredRecords = React.useMemo(() => {
    ...
    if (tab !== 'source' && filters.sources && filters.sources.length > 0) {
      records = records.filter(record => filters.sources!.includes(record.source));
    }

    // Apply user filter (if tab is not already filtering by user)
    if (tab !== 'user' && filters.users && filters.users.length > 0) {
      records = records.filter(record => filters.users!.includes(record.userId));
    }

    // Apply field filter
    if (filters.fields && filters.fields.length > 0) {
      records = records.filter(record => filters.fields!.includes(record.fieldName));
    }
    ...
    <div className="container py-6 space-y-6">
        ...
        <div className="flex items-center gap-2">
          <Dialog open={searchDialogOpen} onOpenChange={setSearchDialogOpen}>
            <DialogTrigger asChild>
              <Button variant="outline">Search by Property ID</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
          ...
          <CardContent className="p-4 pt-0">
            {Object.keys(stats.bySource).length > 0 ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground mt-1">No source data available</p>
              </>
            )}
          </CardContent>
        </Card>
      ...
      <Tabs value={tab} onValueChange={handleTabChange} className="w-full">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          ...
          <div className="text-sm text-muted-foreground">{getTabDescription()}</div>
        </div>
      ...
      {filteredRecords.length > 0 ? (
        ...
        <Alert>
          ...
          <AlertDescription>
            No lineage records match your current filter criteria. Try adjusting your filters or tab
            selection.
          </AlertDescription>
        </Alert>
      )}
