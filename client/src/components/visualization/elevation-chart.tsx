...
export const ElevationChart: React.FC<ElevationChartProps> = ({
  ...
  useEffect(() => {
    ...
    const normalizeY = (y: number) => (
      margin + ((y - yMin) / (yMax - yMin)) * (height - 2 * margin)
    );
    ...
    for (const point of sortedPoints) {
      ...
      const rotatedX = centerX + (x - centerX) * Math.cos(rotationRad) - 
                       (y - centerY) * Math.sin(rotationRad);
      const rotatedY = centerY + (x - centerX) * Math.sin(rotationRad) + 
      ...
      ctx.beginPath();
      ctx.rect(rotatedX - pointSize/2, perspectiveY + pointSize/2, pointSize, normalizedElevation * 20);
      ctx.fill();
    }
