...
const getActivityIcon = (activity: SystemActivity, agents: AIAgent[] = []) => {
  const agentColor = activity.agentId
    ? agents.find(a => a.id === activity.agentId)?.type === 'integration'
      ? 'bg-yellow-500'
      : 'bg-primary-500'
    : 'bg-gray-400';
  ...
  switch (activity.entityType) {
    case 'protest':
      return (
        <div
          className={`h-8 w-8 ${agentColor} rounded-full flex items-center justify-center ring-8 ring-white`}
        >
    ...
    case 'import':
      return (
        <div
          className={`h-8 w-8 bg-secondary-600 rounded-full flex items-center justify-center ring-8 ring-white`}
        >
    ...
    case 'valuation':
      return (
        <div
          className={`h-8 w-8 bg-green-600 rounded-full flex items-center justify-center ring-8 ring-white`}
        >
    ...
    case 'property':
      return (
        <div
          className={`h-8 w-8 bg-yellow-500 rounded-full flex items-center justify-center ring-8 ring-white`}
        >
                ...
                <li key={activity.activityId || activity.id}>
                  <div className="relative pb-8">
                    ...
                    <div className="relative flex items-start space-x-3">
                      <div>
                        <div className="relative px-1">{getActivityIcon(activity, agents)}</div>
                      </div>
                      <div className="min-w-0 flex-1 py-1.5">
                        <div className="text-sm text-gray-500">
                          <span className="font-medium text-gray-900">
                            {getAgentName(activity.agentId, agents)}
                          </span>{' '}
                          {activity.activity}
