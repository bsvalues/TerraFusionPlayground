...
export function FutureValuePrediction({ propertyId, className = '' }: FutureValuePredictionProps) {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="p-4 border rounded-lg">
            ...
            <div className="text-2xl font-bold mt-1">{formatCurrency(finalPrediction.value)}</div>
          </div>
        ...
        <div className="bg-muted p-3 rounded-md flex items-start space-x-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              This prediction has a {Math.round(finalPrediction.confidenceLevel * 100)}% confidence
              level with a range from {formatCurrency(finalPrediction.confidenceInterval.lower)} to{' '}
              {formatCurrency(finalPrediction.confidenceInterval.upper)}. Predictions for longer
              timeframes have lower confidence levels.
            </p>
          </div>
        </div>
