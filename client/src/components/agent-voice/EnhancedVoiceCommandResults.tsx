          ...
          <div className="flex items-center gap-2">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>{formatTimestamp(result.timestamp)}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
            {isExpanded ? (
        ...
        <CollapsibleContent className="px-4 pb-4 border-t">
          <div className="mt-4 space-y-3">
            <div className="flex flex-wrap gap-2">
              ...
              {result.contextId && <Badge variant="outline">{result.contextId}</Badge>}
            </div>
      ...
      <CardContent className="pb-3" ref={resultAreaRef}>
        ...
        <div ref={scrollRef} className="results-container">
          {results.length === 0 ? (
            ...
            <ScrollArea className="h-[350px]">{results.map(renderResult)}</ScrollArea>
          )}
        </div>
      </CardContent>

      <CardFooter>{renderCommandForm()}</CardFooter>
    </Card>
  );
}
