...
import { AlertCircle, CheckCircle2, Info, RefreshCw, Wifi, WifiOff } from 'lucide-react';
...
export function ConnectionStatusIndicator() {
  ...
  const getStatusIcon = () => {
    ...
    switch (status) {
      case 'connected':
        return usingFallback ? <Info className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />;
      case 'connecting':
        return <RefreshCw className="h-4 w-4 animate-spin" />;
      case 'disconnected':
  ...
  const getStatusText = () => {
    switch (status) {
      case 'connected':
        return usingFallback ? 'Connected (Fallback)' : 'Connected';
      case 'connecting':
        ...
        <TooltipTrigger asChild>
          <div className="flex items-center space-x-1 cursor-help">
            <Badge
              ...
              className="flex items-center space-x-1 px-2 py-1 h-7"
            >
              {getStatusIcon()}
