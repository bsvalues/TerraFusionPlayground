...
async function testLLMGeneration() {
  ...
  const prompt = "Explain what CAMA stands for in property assessment and provide a brief description.";
...
async function testLLMAgentIntegration() {
  ...
  const result = await makeRequest(`${API_BASE_URL}/agent-system/property-assessment/analyze/${propertyId}`, 'POST');
