...
export const spatialFeatures = pgTable("spatial_features", {
  id: serial("id").primaryKey(),
  layerId: integer("layer_id").references(() => spatialLayers.id).notNull(),
...
export const gisFeatureCollections = pgTable("gis_feature_collections", {
  id: serial("id").primaryKey(),
  layerId: integer("layer_id").references(() => gisLayers.id).notNull(),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
...
export const insertGISMapProjectSchema = createInsertSchema(gisMapProjects, {
  name: z.string().min(1),
  description: z.string().optional(),
  center: z.array(z.number()).length(2).refine(
    val => val[0] >= -180 && val[0] <= 180 && val[1] >= -90 && val[1] <= 90,
