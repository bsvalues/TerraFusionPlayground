...
export class PluginSettingsManager {
  ...
  getSettings(pluginName: string): PluginSettings {
    ...
    if (fs.existsSync(settingsPath)) {
      try {
        ...
        console.error(chalk.red(`Error reading settings for plugin '${pluginName}': ${error.message}`));
        return {};
      }
    }
    ...
    if (fs.existsSync(manifestPath)) {
      try {
        ...
        console.error(chalk.red(`Error reading manifest for plugin '${pluginName}': ${error.message}`));
      }
    }
  ...
  saveSettings(pluginName: string, settings: PluginSettings): void {
    ...
    try {
      ...
      console.error(chalk.red(`Error saving settings for plugin '${pluginName}': ${error.message}`));
    }
  }
  ...
  async editSettings(pluginName: string): Promise<void> {
    ...
    const { action } = await inquirer.prompt({
      ...
      choices: [
        { name: 'Edit existing settings', value: 'edit', disabled: Object.keys(currentSettings).length === 0 },
        { name: 'Add new settings', value: 'add' },
        { name: 'Delete settings', value: 'delete', disabled: Object.keys(currentSettings).length === 0 },
        { name: 'Reset to defaults', value: 'reset', disabled: Object.keys(settingDefinitions).length === 0 },
   ...
   */
  private async editExistingSettings(pluginName: string, currentSettings: PluginSettings): Promise<void> {
   ...
   * @param defaultSettings Default settings from manifest
   */
  private async resetSettings(pluginName: string, defaultSettings: Record<string, any>): Promise<void> {
  ...
  async listPluginsWithSettings(): Promise<void> {
    ...
    const settingsFiles = fs.readdirSync(this.settingsDir)
