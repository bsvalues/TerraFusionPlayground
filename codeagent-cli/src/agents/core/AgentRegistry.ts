...
import { AgentRegistryEntry, AgentStatus, AgentCapability, AgentType, AgentEventType } from './types';
...
export class AgentRegistry extends EventEmitter {
  ...
  public registerAgent(agent: BaseAgent): boolean {
    try {
      ...
      this.logger.error(`Error registering agent: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  public unregisterAgent(agentId: string): boolean {
    try {
      ...
      this.logger.error(`Error unregistering agent: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
