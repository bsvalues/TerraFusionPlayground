...
export class AdaptiveGisAgent extends AdaptiveAgent {
  ...
  private async optimizeMapRenderingWithLearning(
    ...
    try {
      ...
      this.logger.error(`Error in learning-enhanced map rendering optimization: ${error instanceof Error ? error.message : String(error)}`);
      return standardResult;
    }
  }
  ...
  private extractMapRenderingOptimizations(learningOutput: string): {
    ...
    try {
      ...
      this.logger.error(`Error extracting map rendering optimizations: ${error instanceof Error ? error.message : String(error)}`);
      return {};
    }
  }
  ...
  private async performSpatialQueryWithLearning(
    ...
    try {
      ...
      if (enhancedQuery) {
        ...
        return {
          features: [
            ...
            ...enhancedResult.features.filter(ef => 
              !standardResult.features.some(sf => 
                this.isSameFeature(ef, sf)
              )
            )
          ],
      ...
      this.logger.error(`Error in learning-enhanced spatial query: ${error instanceof Error ? error.message : String(error)}`);
      return standardResult;
    }
  }
  ...
  private extractEnhancedSpatialQuery(learningOutput: string): string | null {
    ...
    const queryMatch = learningOutput.match(/```(?:sql|spatial)\n([\s\S]*?)\n```/) ||
    ...
    const sectionMatch = learningOutput.match(/(?:enhanced|improved) query:?\s*\n?([\s\S]*?)(?:\n\n|$)/i);
  ...
  private async analyzeSpatialPatternsWithLearning(
    ...
    try {
      ...
      this.logger.error(`Error in learning-enhanced spatial pattern analysis: ${error instanceof Error ? error.message : String(error)}`);
      return standardResult;
    }
  }
  ...
  private extractEnhancedSpatialAnalysis(learningOutput: string): any {
    try {
      ...
      this.logger.error(`Error extracting enhanced spatial analysis: ${error instanceof Error ? error.message : String(error)}`);
      return {};
    }
  }
