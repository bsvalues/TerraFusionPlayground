  ...
  if (context.dataContext && context.result) {
    ...
    try {
      ...
      if (processedData) {
        ...
        const { PluginSettingsManager } = await import('../../../../src/plugins/pluginSettingsManager.js');
        ...
        if (processedData.summary) {
          ...
          console.log(`Records processed: ${Array.isArray(processedData.records) ? processedData.records.length : 'N/A'}`);
        }
            ...
            if (typeof sampleRecord === 'object') {
              for (const key in sampleRecord) {
                if (typeof sampleRecord[key] === 'number') {
                  ...
                  const values = processedData.records.map(record => record[key]).filter(v => typeof v === 'number');
