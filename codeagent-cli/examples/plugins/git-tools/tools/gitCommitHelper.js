...
export class GitCommitHelper extends BaseTool {
  ...
  async analyzeCommit(commitHash) {
    ...
    const stats = commitDetails.match(/\\d+ files? changed(?:, (\\d+) insertions?\\(\\+\\))?(?:, (\\d+) deletions?\\(-\\))?/);
    ...
    let type = 'unknown';
    const conventionalMatch = subject.match(/^(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert)(\\(.*\\))?:/);
    ...
    return {
      success: true,
      output: `Commit ${commitHash} Analysis:\\n\\n` +
              `Subject: ${subject}\\n` +
              `Type: ${type}\\n` +
  ...
  async summarizeChanges() {
    ...
    // Add overall stats
    const stats = diffSummary.match(/\\d+ files? changed(?:, (\\d+) insertions?\\(\\+\\))?(?:, (\\d+) deletions?\\(-\\))?/);
  ...
  determineCommitType(files) {
    ...
    const fileTypes = files.map(file => {
      ...
      if (file.includes('test/') || file.includes('spec/') || extension === 'test.js' || extension === 'spec.js') {
        return 'test';
      } else if (file.includes('doc/') || file.includes('docs/') || extension === 'md') {
        return 'docs';
      } else if (file.includes('.github/') || file.includes('ci/') || file === '.travis.yml' || file === 'Jenkinsfile') {
        return 'ci';
