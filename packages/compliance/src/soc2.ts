...
export class SOC2ComplianceManager extends EventEmitter {
  ...
  public runGapAssessment(): { compliant: Control[]; nonCompliant: Control[] } {
    ...
    const compliant = controls.filter(
      control => control.status === ControlStatus.IMPLEMENTED || control.status === ControlStatus.VERIFIED
    );
    
    const nonCompliant = controls.filter(
      control => control.status === ControlStatus.NOT_IMPLEMENTED || control.status === ControlStatus.PARTIALLY_IMPLEMENTED || control.status === ControlStatus.FAILED
    );
