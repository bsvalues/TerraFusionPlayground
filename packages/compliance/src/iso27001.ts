...
export class ISO27001ComplianceManager extends EventEmitter {
  ...
  public generateSOA(): any {
    ...
    const result = {
      ...
      summary: {
        ...
        implementedControls: soaItems.filter(item => item.applicable && item.implementationStatus === ISO27001ControlStatus.IMPLEMENTED).length,
        partiallyImplementedControls: soaItems.filter(item => item.applicable && item.implementationStatus === ISO27001ControlStatus.PARTIALLY_IMPLEMENTED).length,
        notImplementedControls: soaItems.filter(item => item.applicable && item.implementationStatus === ISO27001ControlStatus.NOT_IMPLEMENTED).length
      }
    };
  ...
  public runGapAssessment(): { compliant: ISO27001Control[]; nonCompliant: ISO27001Control[] } {
    ...
    const compliant = applicableControls.filter(
      control => control.status === ISO27001ControlStatus.IMPLEMENTED || control.status === ISO27001ControlStatus.VERIFIED
    );
    
    const nonCompliant = applicableControls.filter(
      control => control.status === ISO27001ControlStatus.NOT_IMPLEMENTED || control.status === ISO27001ControlStatus.PARTIALLY_IMPLEMENTED || control.status === ISO27001ControlStatus.FAILED
    );
  ...
  public generateComplianceReport(): any {
    ...
    const report = {
      ...
      summary: {
        ...
        applicableControls: soaItems.filter(item => item.applicable).length,
        totalRisks: risks.length,
        highCriticalRisks: risks.filter(r => r.level === RiskLevel.CRITICAL || r.level === RiskLevel.HIGH).length
      },
